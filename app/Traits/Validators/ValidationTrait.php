...
trait ValidationTrait
{
    ...
    // clientId may be a numeric id or a UUID string depending on the database
    // environment. Accept string|null to avoid type errors when a UUID is used.
    public function validateUpdateComptePayload(array $data, ?string $clientId = null): array
    {
        $errors = [];
        ...
        if ($hasClient) {
            ...
            if (array_key_exists('telephone', $c) && $c['telephone'] !== null && $c['telephone'] !== '') {
                if (! $this->isValidSenegalPhone($c['telephone'])) {
                    ...
                    // uniqueness check against users table (guard if column absent)
